...
async function loadGroups() {
  try {
    ...
    // 选择默认分组：优先选中第一个聚合分组；若无聚合分组则回退到第一个分组
    if (groups.value.length > 0 && !selectedGroup.value) {
      const firstAggregate = groups.value.find(g => g.group_type === "aggregate");
      handleGroupSelect(firstAggregate || groups.value[0]);
    }
  } catch (error) {
...
async function refreshGroupsAndSelect(targetGroupId?: number, selectFirst = true) {
  ...
  if (targetGroupId) {
    ...
    if (targetGroup) {
      ...
      if (targetGroup.group_type === "aggregate") {
        await loadSubGroups();
      }
      return;
    }
    ...
    <div class="keys-container">
      <div class="sidebar">
        <group-list
          ...
          @refresh-and-select="(id: number) => refreshGroupsAndSelect(id)"
        />
      </div>
      ...
      <div class="main-content">
        ...
        <div class="group-info">
          <group-info-card
            ...
            @copy-success="(group: Group) => refreshGroupsAndSelect(group.id)"
