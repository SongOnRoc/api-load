...
function getSubGroupStatus(subGroup: SubGroupInfo): {
  status: "active" | "disabled" | "unavailable" | "manually_disabled";
  text: string;
  ...
  if (subGroup.enabled === false) {
    return {
      ...
      text: t("subGroups.statusManuallyDisabled"),
      type: "default",
    };
  }
...
const togglingSubGroupId = ref<number | null>(null);

// 切换子分组启用状态
async function toggleSubGroupEnabled(subGroup: SubGroupInfo, enabled: boolean) {
  if (!props.selectedGroup?.id || !subGroup.group.id) {
    return;
  }
  const subGroupId = subGroup.group.id;
  const previousOverride = enabledOverrides.value[subGroupId];
  // 立即写入覆盖层，UI 即时响应（避免等待 API 期间的动画错乱）
  enabledOverrides.value = { ...enabledOverrides.value, [subGroupId]: enabled };
  togglingSubGroupId.value = subGroupId;
  try {
    await keysApi.updateSubGroupEnabled(props.selectedGroup.id, subGroupId, enabled);
    message.success(t("subGroups.toggleEnabledSuccess"));
    // 触发父组件重新拉取 subGroups；watch 会清空 override，computed 用新数据
    emit("refresh");
  } catch (error) {
    // 失败回滚覆盖层
    ...
    if (previousOverride === undefined) {
      ...
      next[subGroupId] = previousOverride;
    }
    enabledOverrides.value = next;
    console.error("Failed to toggle sub group enabled:", error);
  } finally {
    togglingSubGroupId.value = null;
  }
}
...
// 子分组启用状态覆盖层：{ [subGroupId]: enabled }
// 用于本地立即响应 switch 切换，避免等待父组件刷新导致的动画回滚
const enabledOverrides = ref<Record<number, boolean>>({});

// watch props.subGroups 变化时清空覆盖层（父组件已刷新，新数据是权威的）
watch(
  () => props.subGroups,
  () => {
...
const sortedSubGroupsWithPercentage = computed<SubGroupRow[]>(() => {
  ...
  const withPercentage = props.subGroups.map(sg => ({
    ...sg,
    enabled: enabledOverrides.value[sg.group.id ?? 0] ?? sg.enabled,
  ...
  <div class="key-table-container">
    ...
    <div class="toolbar">
      <div class="toolbar-left">
        <n-button type="success" size="small" @click="createModalShow = true">
          <template #icon>
      ...
      <n-spin :show="props.loading || false">
        ...
        <div v-else class="keys-grid">
          <div
            ...
            :class="{
              disabled:
                subGroup.enabled === false || subGroup.weight === 0 || subGroup.active_keys === 0,
              'manually-disabled': subGroup.enabled === false,
            }"
          >
            ...
            <div class="key-bottom">
              <div class="key-stats">
                ...
                <n-tooltip trigger="hover" placement="top">
                  <template #trigger>
                    <n-switch
                      :value="subGroup.enabled !== false"
                      :loading="togglingSubGroupId === subGroup.group.id"
                      size="small"
                      @update:value="(v: boolean) => toggleSubGroupEnabled(subGroup, v)"
                    />
                  </template>
                  {{
                    subGroup.enabled === false
                      ? t("subGroups.enableSubGroup")
                      : t("subGroups.disableSubGroup")
                  }}
                </n-tooltip>
              </div>
...
/* 手动禁用样式：与 weight=0 区分，更明显的灰显 */
.key-card.manually-disabled {
  opacity: 0.5;
  background: var(--bg-disabled);
  border-style: dashed;
}

:root.dark .key-card.manually-disabled {
  background: var(--bg-disabled);
}

.key-card.manually-disabled .display-name,
.key-card.manually-disabled .group-name,
.key-card.manually-disabled .weight-label {
  color: var(--text-disabled);
}
