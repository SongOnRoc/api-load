...
// GroupCopyOverrides defines optional overrides applied when copying a group.
type GroupCopyOverrides struct {
	Name        string
	DisplayName string
	Description string
}

func (s *GroupService) CopyGroup(ctx context.Context, sourceGroupID uint, copyKeysOption string, overrides GroupCopyOverrides) (*models.Group, error) {
	...
	// 复制弹窗可选择覆盖名称/显示名/描述；未提供时回退到默认 "Copy" 后缀
	if name := strings.TrimSpace(overrides.Name); name != "" {
		newGroup.Name = s.generateUniqueGroupName(ctx, name)
	}
	if displayName := strings.TrimSpace(overrides.DisplayName); displayName != "" {
		newGroup.DisplayName = displayName
	} else if sourceGroup.DisplayName != "" {
		newGroup.DisplayName = sourceGroup.DisplayName + " Copy"
	}
	if description := strings.TrimSpace(overrides.Description); description != "" {
	...
	// 聚合分组：复制子分组关联（含权重），保持与源聚合分组一致的子分组树
	if sourceGroup.GroupType == "aggregate" {
		var sourceSubGroups []models.GroupSubGroup
		if err := tx.Where("group_id = ?", sourceGroupID).Find(&sourceSubGroups).Error; err != nil {
			return nil, app_errors.ParseDBError(err)
		}
		for _, sg := range sourceSubGroups {
			copied := models.GroupSubGroup{
				GroupID:    newGroup.ID,
...
// CreateSubGroupParams captures the fields required to create a new standard group and add it as a sub-group.
type CreateSubGroupParams struct {
	Name                string
	DisplayName         string
	Description         string
	Upstreams           json.RawMessage
...
// CreateSubGroupResult contains the newly created group and its sub-group association info.
type CreateSubGroupResult struct {
	Group  models.Group `json:"group"`
	Weight int          `json:"weight"`
}

// CreateSubGroup creates a new standard group and immediately adds it as a sub-group to the specified aggregate group.
func (s *GroupService) CreateSubGroup(ctx context.Context, aggregateGroupID uint, params CreateSubGroupParams) (*CreateSubGroupResult, error) {
	// 1. Verify the aggregate group exists and is of type "aggregate"
	var aggregateGroup models.Group
	if err := s.db.WithContext(ctx).First(&aggregateGroup, aggregateGroupID).Error; err != nil {
	...
	if aggregateGroup.GroupType != "aggregate" {
	...
	// 3. Create the standard group (force channel type to match the aggregate group)
	createParams := GroupCreateParams{
		...
		ChannelType:         aggregateGroup.ChannelType,
		Sort:                params.Sort,
		TestModel:           params.TestModel,
		ValidationEndpoint:  params.ValidationEndpoint,
		ParamOverrides:      params.ParamOverrides,
		ModelRedirectRules:  params.ModelRedirectRules,
		ModelRedirectStrict: params.ModelRedirectStrict,
		Config:              params.Config,
		HeaderRules:         params.HeaderRules,
		ProxyKeys:           params.ProxyKeys,
	}

	newGroup, err := s.CreateGroup(ctx, createParams)
	if err != nil {
		return nil, err
	}

	// 4. Add the new group as a sub-group to the aggregate group
	subGroupInput := []SubGroupInput{
		{GroupID: newGroup.ID, Weight: params.Weight},
	}
	if err := s.aggregateGroupService.AddSubGroups(ctx, aggregateGroupID, subGroupInput); err != nil {
		// If adding as sub-group fails, attempt to clean up the created group
		if delErr := s.db.WithContext(ctx).Delete(&models.Group{}, newGroup.ID).Error; delErr != nil {
			logrus.WithContext(ctx).WithError(delErr).WithField("group_id", newGroup.ID).Error("failed to cleanup group after sub-group association failure")
		}
		return nil, err
	}
	...
	// 5. Invalidate cache
	if err := s.groupManager.Invalidate(); err != nil {
		logrus.WithContext(ctx).WithError(err).Error("failed to invalidate group cache after creating sub-group")
	}

	return &CreateSubGroupResult{
		Group:  *newGroup,
