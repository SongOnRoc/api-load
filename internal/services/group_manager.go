...
func (gm *GroupManager) Initialize() error {
	loader := func() (map[string]*models.Group, error) {
		...
		// Load all sub-group relationships for aggregate groups (only enabled ones with weight > 0)
		var allSubGroups []models.GroupSubGroup
		if err := gm.db.Where("weight > 0 AND enabled = true").Find(&allSubGroups).Error; err != nil {
