...
func (s *Server) ListGroups(c *gin.Context) {
	...
	// 一次性查询每个子分组被多少个聚合分组引用，避免 N+1
	parentCountMap, err := s.getParentAggregateCountMap(c.Request.Context())
	if s.handleGroupError(c, err) {
		return
	}
	...
	for i := range groups {
		resp := s.newGroupResponse(&groups[i])
		if groups[i].GroupType != "aggregate" {
			resp.ParentAggregateCount = parentCountMap[groups[i].ID]
		}
...
// getParentAggregateCountMap 返回 sub_group_id -> 被引用次数 的映射
func (s *Server) getParentAggregateCountMap(ctx context.Context) (map[uint]int, error) {
	type countRow struct {
		SubGroupID uint `gorm:"column:sub_group_id"`
...
type GroupResponse struct {
	...
	// ParentAggregateCount 表示该标准分组被多少个聚合分组引用，仅在列表接口填充，聚合分组本身为 0
	ParentAggregateCount int `json:"parent_aggregate_count"`
}
...
type GroupCopyRequest struct {
	...
	Name        string `json:"name"`       // 可选：覆盖复制后分组名
	DisplayName string `json:"display_name"`
	Description string `json:"description"`
}
...
func (s *Server) CopyGroup(c *gin.Context) {
	...
	newGroup, err := s.GroupService.CopyGroup(c.Request.Context(), uint(id), req.CopyKeys, services.GroupCopyOverrides{
		Name:        req.Name,
		DisplayName: req.DisplayName,
		Description: req.Description,
	})
...
// UpdateSubGroupEnabledRequest 切换子分组启用状态
type UpdateSubGroupEnabledRequest struct {
	Enabled bool `json:"enabled"`
}

// UpdateSubGroupEnabled handles toggling the enabled state of a sub group
func (s *Server) UpdateSubGroupEnabled(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		response.ErrorI18nFromAPIError(c, app_errors.ErrBadRequest, "validation.invalid_group_id")
		return
	...
	var req UpdateSubGroupEnabledRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Error(c, app_errors.NewAPIError(app_errors.ErrInvalidJSON, err.Error()))
		return
	}

	if err := s.AggregateGroupService.UpdateSubGroupEnabled(c.Request.Context(), uint(id), uint(subGroupID), req.Enabled); s.handleGroupError(c, err) {
		return
	}

	response.SuccessI18n(c, "success.sub_group_enabled_updated", nil)
}
...
// CreateSubGroupRequest defines the payload for creating a new standard group as a sub-group of an aggregate group
...
// CreateSubGroup handles creating a new standard group and adding it as a sub-group to an aggregate group
func (s *Server) CreateSubGroup(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		response.ErrorI18nFromAPIError(c, app_errors.ErrBadRequest, "validation.invalid_group_id")
		return
	...
	params := services.CreateSubGroupParams{
		Name:                req.Name,
		DisplayName:         req.DisplayName,
		Description:          req.Description,
		Upstreams:           req.Upstreams,
	...
	result, err := s.GroupService.CreateSubGroup(c.Request.Context(), uint(id), params)
	if s.handleGroupError(c, err) {
		return
	}
